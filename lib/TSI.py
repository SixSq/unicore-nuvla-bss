...
def process_config_value(key, value, config, LOG):
    ...
    """
    # TODO: you probably wanna refactor here
    if 'tsi.enforce_gids_consistency' == key:
        if 'true' == value:
            config['tsi.enforce_os_gids'] = True
        elif 'false' == value:
    ...
    elif 'tsi.fail_on_invalid_gids' == key:
        if 'true' == value:
            config['tsi.fail_on_invalid_gids'] = True
        elif 'false' == value:
            config['tsi.fail_on_invalid_gids'] = False
        else:
            raise KeyError("Invalid value '%s' for parameter '%s', "
                           "must be 'true' or 'false'" % (value, key))
    elif key.startswith('tsi.acl'):
            ...
            if "#" + cmd + "\n" in message:
                ...
                try:
                    if do_set_uid:
                        ...
                        BecomeUser.become_user(user, groups, config, LOG)
